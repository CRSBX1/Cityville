...
void printPerformance(double elapsedMs, long memKB) {
    printSeparator(50, '-');
    cout << fixed << setprecision(6);
    cout << "  Execution Time : " << elapsedMs << " ms" << endl;
    cout << "  Memory Usage   : " << memKB     << " KB" << endl;
    printSeparator(50, '-');
    cout << endl;
}
...
void displaySortResult(Node* originalHead, const string& cityName, const string& algoName, SortField field, SortOrder order, bool useInsertion){
    ...
    //count nodes in the sorted copy to estimate memory used by the sort
    int nodeCount = 0;
    for(Node* n = copy; n != NULL; n = n->next) nodeCount++;
    long memKB = ((long)nodeCount * (long)sizeof(Node)) / 1024;
...
void aStarSearchMenu(Node* cityA, Node* cityB, Node* cityC) {
    ...
    do {
        ...
        cout << "\nAge Group Filter (0 = skip):" << endl;
        cout << "1. Children & Teenagers        (6-17)"   << endl;
        cout << "2. University / Young Adults   (18-25)"  << endl;
        cout << "3. Working Adults Early Career (26-45)"  << endl;
        cout << "4. Working Adults Late Career  (46-60)"  << endl;
        cout << "5. Senior Citizens / Retirees  (61-100)" << endl;
        cout << "0. No age filter" << endl;
        cout << "Select age group: ";
        int ageGroup;
        cin >> ageGroup;
        if(ageGroup < 0 || ageGroup > 5) ageGroup = 0;

        cout << "\nMode of Transport Filter (press Enter to skip): ";
        cin.ignore();
        string transport;
        getline(cin, transport);

        cout << "\nDaily Distance Filter:" << endl;
